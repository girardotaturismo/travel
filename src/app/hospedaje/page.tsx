...
export default async function HospedajePage() {
  ...
  const fincas = displayData.filter((c) => c.subCategory === 'Finca')
      ...
      <section className="bg-primary pt-24 pb-20 rounded-b-[3rem] shadow-xl overflow-hidden relative">
        <div className="container mx-auto px-4 md:px-8 text-center max-w-4xl relative z-10">
          ...
          <p className="text-xl text-white/90 leading-relaxed font-medium">
            Descansa y desconecta en medio de la naturaleza o en el corazón del pueblo. Desde Hoteles cómodos hasta espectaculares Fincas y Glampings exclusivos.
          </p>
        </div>
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1542314831-c6a4d27ce6a2?ixlib=rb-4.0.3')] mix-blend-overlay opacity-10 bg-cover bg-center"></div>
      </section>
        ...
        <Tabs defaultValue="todos" className="w-full">
          <div className="flex justify-center mb-10 w-full overflow-x-auto">
            <TabsList className="h-16 p-1.5 rounded-2xl bg-white border border-border/50 shadow-lg">
              ...
              <TabsTrigger value="fincas" className="px-6 h-12 rounded-xl text-base font-bold data-[state=active]:bg-primary data-[state=active]:text-white">Fincas</TabsTrigger>
          ...
          <TabsContent value="fincas" className="mt-0 outline-none">{renderGrid(fincas)}</TabsContent>
